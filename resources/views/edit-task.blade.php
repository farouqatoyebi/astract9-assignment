            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if (count($taskCategories->all()))
                        <form action="{{ route('modify-user-task', ['id' => $task->id]) }}" method="post" class="form" autocomplete="off">
                            @csrf
                            <div class="form-group">
                                <label for="subject">Title</label>
                                <input type="text" name="title" id="title" value="{{ old('title', $task->title) }}" class="form-control @error('title') is-invalid @enderror" required>
                                ...
                                <select name="category" id="category" class="form-control custom-select @error('category') is-invalid @enderror" required>
                                    ...
                                    <option value="" disabled>- - - -</option>
                                        @foreach ($taskCategories as $category)
                                            <option value="{{ $category->id }}" @if (old('category', $task->category_id) == $category->id) selected @endif>{{ $category->title }}</option>
                                        @endforeach>
                                </select>
                            ...
                            <div class="form-group mt-3">
                                ...
                                <input type="date" name="deadline" id="deadline" class="form-control @error('deadline') is-invalid @enderror" value="{{ old('deadline', date("Y-m-d", strtotime($task->deadline))) }}" required />
